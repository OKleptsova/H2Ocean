SUBROUTINE Interp2D_bivar(x,y,nd,xd,yd,zd,z_out)
    USE Mod_Precision
    IMPLICIT NONE
    REAL(KIND=REAL_DP),INTENT(IN)      :: x, y
    INTEGER(KIND=INT_KIND),INTENT(IN)  :: nd
    REAL(KIND=REAL_DP),INTENT(IN)      :: xd(nd) ,yd(nd) ,zd(nd) 
    ...
    niwk = 31 * nd + ni
    nwk = 8 * nd 
    ALLOCATE(iwk(niwk),wk(nwk))
 
    xi(1) = real(x)
    yi(1) = real(y)  
    md =1
    CALL idbvip (md, nd, real(xd), real(yd), real(zd), ni, xi, yi, zi, iwk, wk )
    z_out = dble(zi(1))
