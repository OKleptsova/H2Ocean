...
SUBROUTINE H2Ocean_Output_Once
  ...
  IF(time_run >= output_start_time .AND. time_run <= output_end_time) THEN
      IF( output_to_bin .OR. output_to_txt ) THEN
          CALL H2Ocean_BIN_TXT_Output_Once
      ENDIF
...
SUBROUTINE H2Ocean_BIN_TXT_Output_Once
  USE Mod_Precision
  USE Mod_All_Variable
  USE Mod_H2Ocean
  USE Mod_TriMesh
  ...
  IF(enable_zeta_output) THEN 
      IF ( output_to_bin ) THEN
         ...
         fid= get_file_unit()
         OPEN(unit=fid,form='UNFORMATTED', file=filename,status="replace")
         DO nd = 1, TriMesh_nodenmb                 
      ...
      IF ( output_to_txt ) THEN
         filename =  trim(adjustl(output_folder))//'eta_'//TRIM(ADJUSTL(xstring))//'.txt'
         fid= get_file_unit()
  ...
  IF(enable_vel_output) THEN 
      ...
      OPEN(unit=fid_vel,form='UNFORMATTED', file=filename,status="replace")
      DO edg = 1, TriMesh_edgenmb                 
  ...
  IF(enable_q_output .AND. enable_nonhydro) THEN 
      ...
      fid_q= get_file_unit()
      OPEN(unit=fid_q,form='UNFORMATTED', file=filename,status="replace")
      DO nd = 1, TriMesh_nodenmb                 
  ...
  IF(enable_w_output .AND. enable_nonhydro) THEN 
      ...
      fid_w= get_file_unit()
      OPEN(unit=fid_w,form='UNFORMATTED', file=filename,status="replace")
      DO nd = 1, TriMesh_nodenmb                 
...
END SUBROUTINE H2Ocean_BIN_TXT_Output_Once
